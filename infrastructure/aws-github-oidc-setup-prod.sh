...
echo "🔐 Setting up GitHub Actions OIDC for cht-companion (production)"
echo "================================================================="
echo ""
echo "⚠️  Run this in the production AWS account (233636046512 for CHT)."
echo ""
...
ROLE_NAME="GitHubActions-CHT-Companion-Prod"
...
echo "  Trust:       GitHub environment \"prod\" only"
echo ""

cat > /tmp/github-trust-policy-cht-companion-prod.json << TRUST
{
  "Version": "2012-10-17",
  "Statement": [
    {
      ...
      "Condition": {
        ...
        "StringLike": {
          "token.actions.githubusercontent.com:sub": [
            "repo:${GITHUB_USER}/${REPO_NAME}:environment:prod",
            "repo:${GITHUB_USER}@${GITHUB_ORG_ID}/${REPO_NAME}@${GITHUB_REPO_ID}:environment:prod"
          ]
        }
      }
    }
...
ROLE_ARN="$(aws iam create-role \
  --role-name "$ROLE_NAME" \
  --assume-role-policy-document file:///tmp/github-trust-policy-cht-companion-prod.json \
  --description "GitHub Actions deploy role for cht-companion (prod environment only)" \
  --query 'Role.Arn' \
...
aws iam update-assume-role-policy \
  --role-name "$ROLE_NAME" \
  --policy-document file:///tmp/github-trust-policy-cht-companion-prod.json
...
echo "  repo:${GITHUB_USER}@${GITHUB_ORG_ID}/${REPO_NAME}@${GITHUB_REPO_ID}:environment:prod"
echo ""
echo "Add this GitHub Environment secret:"
echo "  Environment: prod"
echo "  Name:        AWS_ROLE_ARN"
echo "  Value:       $ROLE_ARN"
echo ""
echo "Used by: deploy-prod.yml, rollback.yml (prod)"
echo ""

rm -f /tmp/github-trust-policy-cht-companion-prod.json
